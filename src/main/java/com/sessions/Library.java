...
public class Library {
    ...
    void removeFromCatalogue(String bookName) {
        ...
        if(null == bookToBeRemoved) {
            throw new BookNotFoundException(bookName + "doesn't exist");
        }
    ...
    public Receipt returnBook(RentedBook rentedBook, Double givenAmount) {

        Double actualAmount = calculateBookRent(rentedBook);

        if(actualAmount > givenAmount)
            throw new LowerAmountException("Given amount should be atleast " + actualAmount);

        Receipt receipt = new Receipt(rentedBook.getBook().getName(), actualAmount, givenAmount);
        books.add(rentedBook.getBook());
        return receipt;
    }
}
